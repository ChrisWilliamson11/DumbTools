...
def import_obj(obj_path):
    ...
    try:
        # Blender 4.0+ OBJ importer with adjusted settings
        ...
        if bpy.context.selected_objects:
            ...
            # Apply scale after import
            imported_obj.scale = (0.01, 0.01, 0.01)
            
            # Rotate 90 degrees on X
            imported_obj.rotation_euler.x = 1.5708  # 90 degrees in radians
            
            # Apply scale and rotation
            bpy.context.view_layer.objects.active = imported_obj
            bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)
            
            print(f"Successfully imported: {imported_obj.name}")
            return imported_obj
        else:
            print("No object was selected after import")
            return None
...
def find_fbx_files(directory):
    """Find all FBX files with 'LOD0' in the filename."""
    fbx_files = []
    for file in os.listdir(directory):
        if file.lower().endswith('.fbx') and 'lod0' in file.lower():
            fbx_files.append(os.path.join(directory, file))
    return fbx_files

def import_fbx(fbx_path, offset_x=0):
    """Import an FBX file and return the imported object."""
    try:
...
def create_material_from_json(json_path):
    try:
        # Find FBX files first
        directory = os.path.dirname(json_path)
        fbx_files = find_fbx_files(directory)
        
        # Skip if no FBX files found
        if not fbx_files:
            print("No LOD0 FBX files found - skipping")
            return None
            
        # Clear everything before starting
        clear_scene()
        
        # Create a new collection for the plant
        ...
        # Import all FBX files with offset
        imported_objects = []
        for i, fbx_path in enumerate(fbx_files):
            objects = import_fbx(fbx_path, offset_x=i * 0.1)
            if objects:
                ...
                for obj in objects:
                    ...
                    # Add to our new collection
                    collection.objects.link(obj)

        if not imported_objects:
            print("Failed to import any FBX files")
            return None

        # Continue with existing material creation code
        # Load JSON data (already loaded above)
        ...
        if imported_obj and material:
            ...
            return imported_obj
                ...
                # After material is created and assigned to objects, mark collection as asset
        collection.asset_mark()
        
        # Add tags to collection instead of individual objects
        semantic_tags = extract_semantic_tags(data)
        if semantic_tags:
            for tag in semantic_tags:
                try:
        ...
        # Set preview image for collection
        preview_path = find_preview_image(directory)
        if preview_path:
            if load_collection_preview(collection, preview_path):
                print("Preview image loaded successfully")
            else:
                print("Failed to load preview image")
...
def load_collection_preview(collection, preview_path):
    """Load a preview image for a collection asset."""
    try:
...
def save_material_to_blend(asset_name, directory):
    ...
    try:
        ...
        blend_path = os.path.join(directory, f"{asset_name}.blend")
...
# Main execution
def main():
    root_folder = "F:/Megascans/3DPlants"
    ...
    for dirpath, dirnames, filenames in os.walk(root_folder):
        ...
        # Check for OBJ file first
        if not any(f.lower().endswith('.obj') for f in filenames):
            print(f"Skipping folder (no OBJ file): {dirpath}")
            continue
