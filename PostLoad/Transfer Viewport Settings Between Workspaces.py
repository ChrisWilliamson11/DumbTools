        ...
        for space in area.spaces:
            if space.type == 'VIEW_3D':
                settings = {
                    ...
                    'local_view_objects': [obj.name for obj in space.local_view.state.objects] if space.local_view else [],
    ...
    if area.type == 'VIEW_3D':
        for space in area.spaces:
            if space.type == 'VIEW_3D':
                ...
                space.overlay.show_overlays = settings.get('overlay_show_overlays', True)
                space.overlay.show_wireframes = settings.get('overlay_show_wireframes', True)
                space.overlay.wireframe_threshold = settings.get('overlay_wireframe_threshold', 1.0)
                space.overlay.show_outline_selected = settings.get('overlay_show_outline_selected', True)
                ...
                space.overlay.show_relationship_lines = settings.get('overlay_show_relationship_lines', True)
                space.overlay.show_bones = settings.get('overlay_show_bones', True)
                space.overlay.show_face_orientation = settings.get('overlay_show_face_orientation', False)
                space.shading.show_xray = settings.get('shading_show_xray', False)
                space.shading.show_xray_wireframe = settings.get('shading_show_xray_wireframe', False)
                space.shading.xray_alpha = settings.get('shading_xray_alpha', 0.5)
                space.shading.xray_alpha_wireframe = settings.get('shading_xray_alpha_wireframe', 0.5)
                ...
                space.show_gizmo_object_translate = settings.get('show_gizmo_object_translate', True)
                space.show_gizmo_object_rotate = settings.get('show_gizmo_object_rotate', True)
                space.show_gizmo_object_scale = settings.get('show_gizmo_object_scale', True)
                space.show_gizmo_empty_image = settings.get('show_gizmo_empty_image', True)
                space.show_gizmo_empty_force_field = settings.get('show_gizmo_empty_force_field', True)
                space.show_gizmo_light_size = settings.get('show_gizmo_light_size', True)
                space.show_gizmo_light_look_at = settings.get('show_gizmo_light_look_at', True)
                space.show_gizmo_camera_lens = settings.get('show_gizmo_camera_lens', True)
                space.show_gizmo_camera_dof_distance = settings.get('show_gizmo_camera_dof_distance', True)
                space.shading.light = settings.get('shading_light', 'FLAT')
                space.shading.wireframe_color_type = settings.get('shading_wireframe_color_type', 'SINGLE')
                space.shading.color_type = settings.get('shading_color_type', 'MATERIAL')
                space.shading.background_type = settings.get('shading_background_type', 'THEME')
                space.shading.show_backface_culling = settings.get('shading_show_backface_culling', False)
                space.shading.show_xray = settings.get('shading_show_xray', False)
                ...
                if settings.get('local_view'):
                    ...
                    local_objects = [bpy.data.objects[obj_name] for obj_name in settings.get('local_view_objects', [])]
                ...
                space.region_3d.view_perspective = settings.get('view_perspective', 'PERSP')
                if settings.get('view_perspective') == 'CAMERA':
                    space.camera = bpy.data.objects.get(settings.get('camera'))
                else:
                    space.region_3d.view_location = settings.get('view_location', space.region_3d.view_location)
                    space.region_3d.view_rotation = settings.get('view_rotation', space.region_3d.view_rotation)
                    space.region_3d.view_distance = settings.get('view_distance', space.region_3d.view_distance)
...
def workspace_changed(*args):
    ...
    if last_workspace_name and last_workspace_name != current_workspace_name:
        ...
        if old_workspace:
            ...
            settings_list = [get_viewport_settings(area) for area in old_workspace.screens[0].areas if area.type == 'VIEW_3D']
        ...
        if old_settings_list:
            old_settings = old_settings_list[0]  # Use the first set of settings as an example
...
# Define the owner of the subscription (can be anything)
owner = object()
