...
class DUMBTOOLS_OT_image_to_spline(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.use_animation:
            self.root_empty = bpy.data.objects.new(f"{img.name}_Root", None)
            context.collection.objects.link(self.root_empty)
            self.root_empty.location = obj.location
            
            self.hide_empty = bpy.data.objects.new(f"{img.name}_Hide", None)
            context.collection.objects.link(self.hide_empty)
            self.hide_empty.parent = self.root_empty
            self.hide_empty.location = (0, 0, -100) # local offset
            ...
            self.process_frame(context, self.original_frame)
            self.report({'INFO'}, f"Successfully created mesh from {img.name}")
            return {'FINISHED'}
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            # Yield to Blender's event loop for a few ticks to let background video decoders load the frame
            self.wait_ticks += 1
            if self.wait_ticks < 2:
                return {'PASS_THROUGH'}
                
            self.process_frame(context, self.current_frame)
    ...
    def cancel(self, context):
        ...
        self.report({'WARNING'}, "Animation processing cancelled.")
        return {'CANCELLED'}
        
    def finish(self, context):
        context.window_manager.event_timer_remove(self.timer)
        context.scene.frame_set(self.original_frame)
        self.report({'INFO'}, f"Successfully created {self.meshes_created} mesh(es).")
        return {'FINISHED'}

    def process_frame(self, context, f):
        img = self.img
        cv2 = self.cv2
        ...
        if img.source == 'SEQUENCE' and self.node and hasattr(self.node, "image_user"):
            filepath = ""
            if hasattr(img, "filepath_from_user"):
                try: filepath = bpy.path.abspath(img.filepath_from_user(image_user=self.node.image_user))
                except: pass
            ...
            if m:
                image_frame = f - self.node.image_user.frame_start + self.node.image_user.frame_offset + 1
                frame_str = m.group(1)
        ...
        if self.use_animation:
            curve_obj.parent = self.root_empty
            curve_obj.location = (0, 0, 0)
        else:
            curve_obj.location = self.obj.location
        ...
        # Assign material
        new_mesh_obj.data.materials.append(self.mat)
        ...
        if self.use_animation:
            ...
            constraint.target = self.hide_empty
